...
import { rollRun, finishRun, getMeta, resetMeta } from './meta.js';
...
export function newRun(){
  ...
  // WARIANTY POLA + ESKALACJA — po resecie S.* (mogą ruszyć U/B/BAL/S.eBase/S.pBonus
...
async function main(){
  ...
  app.ticker.add(()=>{
    ...
    // przejście play → koniec runu: zbierz raport RAZ (metryki są w S.stat)
    const ended = S.state==='win' || S.state==='over';
    if (ended && prevState!=='win' && prevState!=='over') finishRun();
    prevState = S.state;
    renderFrame();
    updateHUD();
    syncOverlays();
